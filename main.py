...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_net', type=float, default=1e-3, help="initial learning rate")  #原来1e-3
    ...
    parser.add_argument("--style_size", type=int, default=512, help="风格图片的大小，默认为原图大射1�7")
    parser.add_argument("--style_layers", type=str, default='-1', help="图片风格特征提取的层的索引序刄1�7,[3,8,15,22]或[-1]")
    parser.add_argument("--style_weight", type=float, default=5.0, help="风格损失的权釄1�7")
    parser.add_argument('--style_target_type', default='gram', choices=['gram', 'mean'])
    #  Options for feature reconstruction loss

    parser.add_argument("--content_layers", type=str, default="8", help="图片内容特征提取的层的索引序刄1�7,默认8")
    parser.add_argument("--content_size", type=int, default=512, help="训练图片的大小，默认丄1�7 512*512")
    # vgg model
    parser.add_argument("--arch", type=str, default="vgg16", help="作图片特征提取的网络名称，目前支持的朄1�7 vgg16, vgg19")
    ...
    parser.add_argument('--use_percep_loss', default=True, type=str2bool,help="True,False")
    # 音频
    parser.add_argument('--video_aud', default='', type=str, help="video_audio")

    # 情绪标签
    parser.add_argument('--emo',  default=False, type=str2bool,help="情绪标签，True or False")
    ...
    # 对比学习
    parser.add_argument('--contrast_loss', default=True,type=str2bool,)
    parser.add_argument('--rev_aud',
                        default='/mnt/sda/zpf/testdata/aud_ave_e_m.npy')
    # parser.add_argument('--rev_aud',
    #                     default='/data15/guojb2401/CZGNerf/testdata/aud_ave_e_m.npy')
    # 对躯干的新的采样策略
    parser.add_argument('--train_torso', default=False, type=str2bool,help="True or False")
    parser.add_argument('--torso_patch_size', default=16, help="size")


    # aud_ch_att_net网络进行注意力优匄1�7   音频对采样点的注意力网络
    parser.add_argument('--aud_ch_att_net', type=str, default='Mamba1', help='LineModel,MWSE,MWDN,MLP,Mamba2,Mamba1')
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            # metrics = [PSNRMeter(), LPIPSMeter(device=device), LMDMeter(backend='fan'),MSSSIMMeter(size_average=True),FIDMeter(device=device),NIQEMeter(device=device)]
            metrics = [PSNRMeter(), LPIPSMeter(device=device), LMDMeter(backend='fan'), MSSSIMMeter(size_average=True),
                       FIDMeter(device=device), NIQEMeter(device=device), BRISQUEMeter(device=device),
                       AUEMeter(device=device), LSEC(device=device),LSED(device=device)]
        ...
        model.emo_labels = test_loader._data.emo_one_hot
        if opt.gui:
        ...
        #optimizer = lambda model: torch.optim.NAdam(model.get_params(opt.lr, opt.lr_net), betas=(0, 0.99), eps=1e-8)
        ...
        # TODO 独热编码 不用的话记得改回厄1�7
        model.emo_labels = train_loader._data.emo_one_hot
