...
def vis_parsing_maps(im, parsing_anno, stride, save_im=False, save_path='vis_results/parsing_map_on_im.jpg',
    ...
    vis_head = vis_parsing_anno_color.copy()
    vis_head = vis_head.astype(np.uint8)
    vis_head_save = cv2.resize(vis_head, img_size,
                        interpolation=cv2.INTER_NEAREST)
    output_head_path = os.path.join(folder_path, f"{os.path.splitext(file_name)[0]}_face.png")
    if save_im:
        cv2.imwrite(output_head_path, vis_head_save)
    # print(os.path.join(folder_path, f"{os.path.splitext(file_name)[0]}_face.png"))
