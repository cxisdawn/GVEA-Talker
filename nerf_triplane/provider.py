...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1):
            ...
            if self.opt.contrast_loss:
                ...
                if self.opt.asr_model == 'ave':
                    ...
                    if len(rev_aud_features.shape) == 3:
                        rev_aud_features = rev_aud_features.float().permute(1, 0,
                                                                            2)  # [1, 2999, 512] --> [2999, 1, 512]
                else:
        ...
        self.emo_one_hot = []
        # # TODO 读取情绪标签和特征
        if (self.opt.emo or self.opt.contrast_loss) and self.opt.aud == '':

            emo_dict = np.load(os.path.join(self.root_path, 'emotion_label.npy'),
                                 allow_pickle=True).item()  # 一个字典{label:{id:label},feats:{id:features}} id从0开始
            emo_labels = emo_dict['label']  # label:{id:label}
            emo_feats = emo_dict['feats']  # feats:{id:features}
        elif (self.opt.emo or self.opt.contrast_loss):  # 读取音频文件所在目录下的标签
            # 提取目录路径
            # print('1111111111111111111111')
            directory_path = os.path.dirname(self.opt.aud)
            emo_dict = np.load(os.path.join(directory_path, 'emotion_label.npy'),
                                 allow_pickle=True).item()  # 一个字典{label:{id:label},feats:{id:features}} id从0开始
            emo_labels = emo_dict['label']  # label:{id:label}
            emo_feats = emo_dict['feats']  # feats:{id:features}
        for f in tqdm.tqdm(frames, desc=f'Loading {type} data'):
            ...
            if (self.opt.emo or self.opt.contrast_loss) and self.opt.aud == '':
                ...
                per_emo_label = emo_labels[min(f['aud_id'], aud_features.shape[0] - 1)]  # careful for the last frame...
                emo_one_hot = np.zeros(9)
                # 将对应位置设置为1
                emo_one_hot[per_emo_label - 1] = 1
                # print(per_emo_label)
                per_emo_feats = emo_feats[min(f['aud_id'], aud_features.shape[0] - 1)][0]  # [1024]

                self.emo_labels.append(per_emo_label)
                self.emo_feats.append(per_emo_feats)
                self.emo_one_hot.append(emo_one_hot)
            elif (self.opt.emo or self.opt.contrast_loss):  # 读取音频文件所在目录下的标签
                # 提取目录路径
                # print('1111111111111111111111')
                #
        ...
        # 提取情感标签
        if self.opt.emo or self.opt.contrast_loss:
            self.emo_labels = np.array(self.emo_labels, dtype=np.float32)  # [N1]
            # print(self.emo_labels.shape)
            self.emo_labels = torch.from_numpy(self.emo_labels).view(-1, 1)  # [N, 9]
            # print(self.emo_labels.shape)
            self.emo_feats = np.array(self.emo_feats,dtype=np.float32)    # [N,1,1024]
            self.emo_feats = torch.from_numpy(self.emo_feats)  # [N, 1]

            self.emo_one_hot = np.array(self.emo_one_hot, dtype=np.float32)
            self.emo_one_hot = torch.from_numpy(self.emo_one_hot)
            # print(self.emo_one_hot.shape)   #[N,9]
            # print('emo_feats',self.emo_feats.shape)  # emo_feats torch.Size([100, 1, 1024])
            # print('emo_label_num:', self.emo_labels.shape)# emo_label_num: torch.Size([100, 1])
        ...
        if self.preload > 1:
            ...
            if self.opt.emo or self.opt.contrast_loss:
                ...
                self.emo_feats = self.emo_feats.to(self.device)
        # load intrinsics
        ...
        # directly build the coordinate meshgrid in [-1, 1]^2
        self.bg_coords = get_bg_coords(self.H, self.W, self.device)  # [1, H*W, 2] in [-1, 1]
        if self.opt.train_torso is True and self.opt.torso:
            self.train_torso = True
        else:
            self.train_torso = False
    ...
    def collate(self, index):
        ...
        if self.auds is not None:
            ...
            if self.opt.contrast_loss:
                ...
                act_auds = get_audio_features(self.auds, self.opt.att, index[0]).to(self.device)

                # 只取一帧
                # rev_auds = get_audio_features(self.rev_auds, 0, index[0]).to(self.device)
                # act_auds = get_audio_features(self.auds, 0, index[0]).to(self.device)
                results['rev_aud'] = rev_auds
                results['act_aud'] = act_auds
                # print(act_auds.shape)
        ...
        if self.training and self.opt.finetune_lips:
            ...
            # print(self.train_torso)
            rays = get_rays(poses, self.intrinsics, self.H, self.W, self.num_rays, self.opt.patch_size,
                            self.opt.torso_patch_size, train_torso=self.train_torso, torso_img=self.torso_img)
        ...
        # TODO 读取处理好的emo信息
        if self.opt.emo or self.opt.contrast_loss:
            # results['emo_label'] = self.emo_labels[index].to(self.device)  # [1,1]
            # print(results['emo_label'].shape)  # [1,1]
            results['emo_feat'] =  self.emo_feats[index].to(self.device)  # [1,1024]

            results['emo_label'] = self.emo_one_hot[index].to(self.device)
            # print(1111111)
            # print(results['emo_one_hot'].shape)  # [1,9]
            #print(results['emo_feat'].shape)  # [1,1024]
            # print(results['emo_label'].shape)  # [1,1]
        # load bg
        # bg_torso_img ，torso_img纯躯干图
        # bg_img纯背景图
        # bg_torso_img = self.torso_img[index]
